...
**Files:**
- Replace: `src/endless_war/ui/tray.py` (currently a two-line placeholder comment from the original skeleton)
- Replace: `src/endless_war/ui/app.py` (currently a 7-line placeholder whose `run()` raises `SystemExit("GTK UI not implemented yet…")`)
...
- **`ui/app.py` and `ui/tray.py` already exist as skeleton placeholders.** Replace their contents wholesale; there is nothing in them to preserve. The old `app.run()` that raises `SystemExit` goes away — nothing imports it (confirm with a grep before deleting, and say so in your report).
