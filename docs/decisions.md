...
- **Final-review minors, fixed afterwards:** the new constants live in `[balance]` (`garrison_broken_share`, `reserve_strength_weight`, `min_new_army`, `new_army_*`); army ids come from a `next_army_id` counter and are never reused; garrison losses are reported as `garrison_losses`, not as dead soldiers; surrender is decided for every army before any is removed, and only to an unbroken enemy; a broken army stranded on unsupplied ground with no route to supply demobilises into the reserves; an annexed capital moves to the faction's most populous province; only a broken army may fall back while disorganized; a destroyed faction reads "destroyed" in the status panel and tray and leaves the legend. **Not applied:** limiting how many wars a *target* can be in — it shielded collapsing factions (a 243-day island at seed 99, gate FAIL); the limit stays on the declarer, and the docs now say so. Gate after these: PASS on 42, 7 and 99.
