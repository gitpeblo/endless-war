...
The map opens at the scale at which the land exactly fills its area; the sea ring may run off the edges. The scroll wheel zooms in whole-scale steps around the cursor, and a middle-button drag pans (`iso.Camera`, `zoom_at`, `pan_by`, `refit`); a resize re-fits or re-clamps the camera. This is widget state only and never reaches the simulation.
